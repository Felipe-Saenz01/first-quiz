...
SELECT COUNT(age) 
FROM animals
WHERE animal_id IN(SELECT pet_id FROM people_animals)
AND age > (
        SELECT age
        FROM people
        WHERE person_id IN(SELECT owner_id FROM people_animals)
    )
...
SELECT p.name AS 'person name', a.name AS 'pet name', a.species AS species
FROM people AS p
